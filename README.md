...
| Economic calculation | `calculator/production.py` calculates deployment, job capacity, income claims and productive capacity. `labor_market.py` tracks workers, matches them to jobs and updates wage contracts for scalar and NumPy calculations. `trade.py` calculates bilateral trade, producer and consumer prices, and trade-adjustment exposure. `settlement.py` pays retained wages, taxes and benefits and checks resource accounting. `model.py` runs the ten-year profiles and computes utilities. |
