...
The US-only ballot contains all 21,384 valid combinations of six policy terms (19,008 when pausing AI is unavailable). International mode adds a trade choice, giving each side 42,768 combinations (38,016 without a pause). The current-policy package remains on the US ballot:
...
3. Set the modeled benefit budget to 0%, 50%, 100%, 150% or 200% of its 2025 reference, or distribute all revenue after public services.
...
6. Set an additional AI-profits tax from 0% to 100% in ten-point steps; ordinary investment tax rates remain fixed.
7. In international mode, allow or ban trade with the other economy.

“Pausing AI isn’t possible” is unchecked by default. When checked, it excludes Pause AI from the US ballot and, in international mode, from the foreign actor’s menu. Downloads and precomputed keys include this setting. Reset restores the unchecked default. The no-AI baseline remains a diagnostic reference, not an available policy.

Noncapital tax choices are 0%, 10%, …, 100%, plus a separate current-reference option. AI-profits tax choices are 0%, 10%, …, 100%, with 0% in the status quo. Each group's tax profile changes toward those endpoints; the page distinguishes the selected benchmark from the resulting average rate after incomes change. References are model allocations of Census-estimated federal/state income taxes and payroll contributions, including self-employment contributions. They are not statutory rates or all US taxes. Negative net tax amounts are represented as benefit payments without double counting.
...
Employer retention is paid from capital resources and creates no extra production while workers remain in replaced roles. Retained workers do not join the external search pool; employers can redeploy them into productive vacancies before outside hires. Protected wages stay tied to pre-AI pay. A package is eligible only when employers can pay promised retained wages and taxes can pay all promised benefits plus the fixed commitment to other public spending in every year. Later surpluses cannot finance earlier deficits; borrowing is not modeled. Eligibility is checked after behavioral responses. Underfunded packages cannot receive votes. Under the default majority rule, current policy remains the automatic fallback if no eligible package wins a majority, even if it becomes underfunded; the result then shows the shortfall. Under the most-votes rule, only a fully funded package other than US current policy can take office. If no such package exists, the calculation reports an error rather than inventing a fallback. Fixed benefit budgets leave surplus receipts with non-transfer public spending. The explicit all-revenue option instead distributes every remaining dollar after reference public services, using the chosen formula.

## Voting and international competition

Every citizen begins with the fully funded package giving their household the greatest ten-year income utility, but can support a strategically preferable compromise through the coordination protocol below. By default, the package with the most votes passes only if **more than half** the population chooses it; otherwise the exact current-tax/current-benefit package with current AI pace remains. The public simulator always uses this majority rule. The page starts with default assumptions; controls keep their state in memory without reading or updating scenario URLs. There is one vote, with no runoff. Utility uses a 3% discount rate, a logarithm and a small offset at zero income. Each income cell has 100 fixed career ranks, shared across counterfactuals. Deterministic ranks follow layoffs, retention, search, hiring and competitive replacement. Voters know their sampled trajectory. Budgets integrate continuous employment shares exactly; the vote is a whole-person approximation of the distribution of known careers. Annual largest-remainder allocation bounds each employment-state share error to one percentage point; rank resolution remains a model limitation. Within a job pool, contractual wages retain the aggregate model’s average factor.

`strategic_ballot.py` considers enactable compromises by a conservative estimate of the benefiting coalition, never policy-ID order. It also allows compromise voters to abstain when doing so restores a preferred status quo. The majority denominator remains all citizens. Ties use policy-terms hashes, with eligible current policy preferred on personal indifference. After a cycle or 64 switches the funded recorded outcome with the smallest strongest-challenger support is selected, then greatest ballot support. That is a rule-selected outcome, not a stable equilibrium. This is a specified coalition protocol, not exhaustive strong-Nash analysis. Arbitrary multi-package coalition arrangements remain outside its stability claim.
...
Run `node scripts/python-tool.mjs python scripts/audit-model.py` for reproducible sensitivity checks at 50/100/200 career paths, linear/log income utility, 0%/3%/10% discounting, a 20-year horizon, and all three foreign objectives. Defaults remain 100 paths, logarithmic income, 3% discounting and ten years. The result file records the model fingerprint. Career paths are numerical representatives, not additional voting weight. The strongest-coalition process is a protocol assumption; the site distinguishes its stable outcomes from cycle-resolution results.

## Additional AI-profits tax

Ordinary investment taxes remain fixed at each cohort’s calibrated rate. The new tax is applied to positive additional profits after ordinary tax: `AI tax = rate × max(0, actual profit − max(0, no-AI profit)) × (1 − ordinary rate)`. Actual profits deduct wages, retained payroll, AI installation, ongoing operating and adjustment costs, and include net foreign profit flows. The annual reference reruns both economies with AI paused, no retention, the same noncapital taxes and the same trade permissions, in real purchasing-power units. This captures productivity gains and wages saved without taxing background growth alone. It is an economy-wide attribution assumption, not firm tax accounting; loss carryforwards and firm-specific cost categories are not modeled separately. Ongoing AI resource costs are an illustrative 2% of AI-exposed output per year, in addition to installation costs; this coefficient is an assumption, not an empirical estimate.

The additional tax reduces private AI deployment by `(1 − rate) ** investmentResponse`; 100% stops private domestic deployment even when the response parameter is zero. Imported spillovers may remain under open trade. The tax does not impose a direct penalty on non-AI capacity. Mobile international AI returns respond to the tax as well. These are declared reduced-form incentive assumptions, not an endogenous firm investment equilibrium.
