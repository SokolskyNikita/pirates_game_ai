...
export function renderMethod() {
  el('model-method').innerHTML =
    ...
    '<p>Each citizen chooses one fully funded complete package maximizing their personal ten-year income utility. By default, strictly more than half must choose the same package; otherwise current policy remains. Checking “Retaining status quo isn’t an option” removes the exact current-policy package from the US ballot and makes the remaining fully funded package with the most votes win at any share, with no automatic fallback. Personal utility ties prefer eligible current policy, then a fixed policy-ID ordering; tied vote totals use that ordering. Everyone knows the rule. In international mode, each side’s choice must be consistent with the other’s known choice. The foreign actor can still choose its own current policy if fully funded.</p>' +
    MODEL_NOTES.map(
      (note) =>
        '<h4>' +
        note.title +
