...
export const INPUT_SPECS: readonly InputSpec[] = [
  {key:'usGdpGrowth',label:'US GDP growth/year at max AI proliferation',min:0,max:.2,step:.05,description:'Annual real output growth at full AI exposure, before policy effects. Growth compounds each year and scales with AI exposure during deployment.'},
  {key:'foreignGdpGrowth',label:'Foreign GDP growth/year at max AI proliferation',min:0,max:.2,step:.05,description:'The foreign bloc’s annual real output growth at full AI exposure, before policy effects. Independent of the US growth assumption.'},
  ...
  {key:'tradeIntensity',label:'US import exposure',min:0,max:1,step:.05,description:'AI spillover exposure anchored to 2025 US imports / GDP; the spillover mechanism is assumed.'},
  {key:'foreignMarketSize',label:'Rest-of-world GDP / US GDP',min:.25,max:8,step:.25,description:'2025 nominal GDP ratio, used for international rent accounting.'},
  ...
  {key:'foreignTradeIntensity',label:'Foreign exposure to US exports',min:0,max:1,step:.05,description:'2025 US exports / rest-of-world GDP. Trade within the foreign bloc is internal.'},
];
...
export const PACE_CHOICES=[0,1,2] as const;
export const PACE_LABELS:Readonly<Record<number,string>>={0:'Pause AI',1:'Allow current AI pace',2:'Accelerate AI'};
export function makePolicy(values:Omit<Policy,'id'|'label'>):Policy {
  ...
  return {...values,id,label:`${PACE_LABELS[pace]??`${pace}× AI pace`} · ${replacement?`Retain at ${Math.round(replacement*100)}%`:'Allow layoffs'} · benefits ${welfareScale*100}% · ${benefitFormula} · noncapital ${Math.round(laborTax*1000)/10}% / investment ${Math.round(capitalTax*1000)/10}% tax`};
}
...
export function policiesAtPace(pace=1):readonly Policy[] {
  if(!PACE_CHOICES.some(p=>Math.abs(p-pace)<1e-9))throw new RangeError('AI pace must be 0 (pause), 1 (current) or 2 (accelerated).');
...
function checkedPolicy(p:Policy):void {
  if(![p.pace,p.replacement,p.welfareScale,p.laborTax,p.capitalTax].every(Number.isFinite)||p.pace<0||p.pace>2||p.replacement<0||p.replacement>1.25||p.welfareScale<0||p.welfareScale>2||p.laborTax<0||p.laborTax>1||p.capitalTax<0||p.capitalTax>1||!BENEFIT_FORMULAS.includes(p.benefitFormula)) throw new RangeError('Invalid policy.');
}
function aiExposure(pace:number,adoption:number,foreignAdoption:number,trade:number,importProgress:number):number {
  // A pause prohibits both domestic deployment and imported AI use. Foreign
  // deployment can still change returns through the separate rent calculation.
  return pace===0?0:clamp(adoption+trade*clamp(importProgress)*foreignAdoption*(1-adoption));
}
function burden(inputs:ModelInputs,p:Policy,otherPace:number,otherStrength:number,strength:number,trade:number,c:Calibration):number {
  // Compare policies on the same ten-step nominal rollout. Keeping the advance
  // burden reference independent of positive speed makes acceleration exactly
  // a calendar compression of the same adoption curve. Actual recovery and
  // installation costs are computed later, on calendar years.
  let affected=0,previous=0;
  for(let y=1;y<=YEARS;y++) {
    const own=deploymentAtYear(strength,p.pace===0?0:1,y,0,0);
    const other=deploymentAtYear(otherStrength,otherPace===0?0:1,y,0,0);
    const exposure=aiExposure(p.pace,own,other,trade,y/YEARS);
    affected=affected*(1-inputs.reemployment)+inputs.displacement*Math.max(0,exposure-previous);
    previous=exposure;
  }
...
function deploymentAtYear(target:number,pace:number,year:number,response:number,burden:number):number {
  const t=clamp(pace*year/YEARS);
  // For response × burden in [-1, 1], this path is monotone and reaches its
  // target in year ten at current speed, or year five at accelerated speed.
...
function produce(inputs:ModelInputs,p:Policy,old:TrajectoryState,adoption:number,foreignAdoption:number,b:number,trade:number,year:number,c:Calibration,gdpGrowth:number):Production {
  const exposure=aiExposure(p.pace,adoption,foreignAdoption,trade,p.pace*year/YEARS);
...
/** Eligibility requires every employer and government promise to be funded. */
...
function settle(policy:Policy,p:Production,year:number,flow:number,prepared:Prepared,materialize:boolean):Settlement {
  ...
  const fullyFunded=requiredEmployer-employerPay<1e-7&&welfareGap<1e-7&&governmentGap<1e-7;
  ...
  if(materialize)point={
    ...
    resourceResidual:allIncome+nonTransferSpending-grossResources,feasible:fullyFunded,
  };
  return {point,utilities,workerScore,prosperityScore:prosperity,outputScore:p.output/100-1,admissible:fullyFunded};
}
...
function simulate(inputs:ModelInputs,usPolicy:Policy,foreignPolicy:Policy|undefined,foreignObjective:Objective,prepared:Prepared,materialize:boolean,foreignOnly=false):LightProfile|ProfileOutcome {
  ...
  const bUS=burden(inputs,usPolicy,foreignPolicy?.pace??0,inputs.foreignStrength,1,trade,c);
  const bForeign=foreignPolicy?burden(inputs,foreignPolicy,usPolicy.pace,1,inputs.foreignStrength,inputs.foreignTradeIntensity,c):0;
  for(let year=1;year<=YEARS;year++) {
    const adoptUS=deploymentAtYear(1,usPolicy.pace,year,inputs.investmentResponse,bUS);
    const adoptForeign=foreignPolicy?deploymentAtYear(inputs.foreignStrength,foreignPolicy.pace,year,inputs.investmentResponse,bForeign):0;
...
export const MODEL_NOTES:readonly {title:string;detail:string;equation?:string}[]=[
  ...
  {title:'Taxes and the budget',detail:'Noncapital income includes work, private pensions and other noninvestment income. Investment income is taxed separately. Current cohort tax-rate differences remain at the current benchmark. Lower benchmarks scale rates toward zero; higher benchmarks scale them toward 100%, so the endpoints apply to every cohort. The benchmark equals the aggregate effective rate on unchanged tax bases; the actual rate can change as incomes change. Negative baseline taxes become modeled net refund benefits. A policy is eligible only if taxes fund both the fixed baseline nontransfer spending requirement and the entire chosen benefit budget in every year. Employer-funded retention promises must also be covered in every year, after the modeled effects on investment, output and international returns. Any excess goes to nontransfer public spending, not an undisclosed household dividend. Voters value their private resources; they do not receive utility from this other public spending.'},
  {title:'Actual payments',detail:'Employer retention is paid from available investment income before household taxes. Public payments are capped by tax revenue after other spending. Underfunded packages cannot be chosen on the ballot. Diagnostic comparisons and the automatic status-quo fallback can still show shortfalls; their income figures use actual payments rather than inventing funds. Household resources plus nontransfer government spending equal available production after installation costs, adjustment costs and foreign rent flows.'},
  ...
  {title:'Annual GDP growth',detail:'US and foreign potential output compound independently at their chosen annual growth rates multiplied by that year’s AI exposure. Exposure includes imported AI only where deployment is allowed. A pause prohibits domestic and imported AI use for all ten years, so it creates no AI job losses or AI growth. The no-AI reference assumes no growth; current-pace deployment ramps over ten years, so a 5% full-AI rate does not mean ten years of 5% growth. Displacement changes who receives production income rather than automatically destroying the production AI replaces. Policy responses can raise or lower realized GDP relative to this potential path. Growth scales productive wages, pensions and other passive resources; benefit budgets and retained-wage promises remain fixed in real baseline dollars.',equation:'Potential outputₜ = potential outputₜ₋₁ × (1 + annual growth at full AI × exposureₜ)'},
  {title:'Production and policy responses',detail:'The production index starts at 100 and is allocated using observed household income components; it is not a forecast of dollar GDP. Work resources respond to displacement and work incentives. Pension and other resources face aggregate capacity changes but are not directly laid off. Current AI pace reaches full domestic deployment in year ten; acceleration compresses the same domestic deployment curve into five years, bringing forward job replacement and the full annual productivity growth rate. Imported AI also requires domestic adoption capacity: its diffusion reaches the chosen import-exposure limit in year ten at current pace or year five when accelerated, and is further limited by the other region’s available AI. Total exposure therefore depends on both regions’ choices. Faster installation has higher annual adjustment and installation costs. Retention costs and tax increases relative to current rates delay deployment within the chosen horizon and discourage capacity renewal; tax reductions can advance deployment. To compare speeds consistently, the policy burden uses a common ten-step nominal rollout before the timing response. Recovery from displacement still occurs on calendar years. Foreign deployment is also limited by frontier capability. The assumed annual renewal share is 5%, not an estimated capital-stock model.'},
  {title:'International assumptions',detail:'The foreign bloc chooses its own full policy and AI deployment pace to maximize worker income, average income utility or output. A pause blocks AI use and AI job replacement within that region even if the other side deploys, but international rent competition can still change income. Where AI is allowed, import diffusion scales with the recipient’s pace as well as the supplier’s available AI; acceleration cannot import AI before the supplier deploys it. If both pause, neither region deploys for the entire ten-year horizon. GDP weights cross-border rent flows. For comparability, its household distribution and baseline fiscal system use the same US-calibrated cohort structure; this is an explicit simplification, not foreign microdata.'},
  {title:'One vote on a complete package',detail:'Each citizen chooses the fully funded package that maximizes their own expected household-resource utility. The package with the largest support wins only if it receives strictly more than half of adult-citizen population weight; otherwise current US policy continues. There is no second ballot. In international scenarios, each side evaluates its choice knowing the other side’s policy; a reported mutually consistent outcome must reproduce those choices. All policies last for the modeled ten years.'},
];
