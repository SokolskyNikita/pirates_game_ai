...
def tally_package_ballot(
    ...
    Underfunded current policy is ineligible for votes but remains the fallback.
    """
    if not len(weights):
    ...
    winner_id = leading["policyId"] if leading and package_has_majority(leading["supportPercent"]) else None
    if leading is None:
    ...
    return {
        ...
        "unfundedCandidateCount": len(ids) - len(eligible_ids),
    }
